...
namespace VstNetAudioPlugin.UI
{
  public partial class PluginEditorView : UserControl
  {
    ...
    internal bool InitializeParameters(IList<VstParameterManager> parameters)
    {
      ...
      for (int i = 1; i <= 24; i++)
      {
        checkedListBox1.Items.Add($"{i}/12");
      }
    ...
    {
      DelayParameters.Notes = this.checkedListBox1.CheckedItems.Cast<string>().Select(x => int.Parse(x.Split('/').First())).ToList();
    }

    private void trackBar1_ValueChanged(object sender, System.EventArgs e)
    {
      DelayParameters.Saw = Math.Pow((double)this.trackBar1.Value / this.trackBar1.Maximum, 1.0 / 3.0);
    }

    private void attack_ValueChanged(object sender, EventArgs e)
    {
      DelayParameters.Attack = (double)this.attack.Value / this.attack.Maximum;
    }

    private void release_ValueChanged(object sender, EventArgs e)
    {
      DelayParameters.Release = (double)this.release.Value / this.release.Maximum;
    }

    private void FmModulation_CheckedChanged(object sender, EventArgs e)
    {
      DelayParameters.FmMod = this.FmModulation.Checked;
    }
  }
}
